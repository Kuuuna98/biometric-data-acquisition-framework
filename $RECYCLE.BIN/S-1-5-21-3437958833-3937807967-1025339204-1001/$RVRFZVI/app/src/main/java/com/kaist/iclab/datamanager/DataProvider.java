...
public class DataProvider extends ContentProvider {
   ...
   // public static final Uri CONTENT_URI_Phone_ACC_SEL = Uri.parse("content://" + PROVIDER_NAME + "/Phone_ACC_SEL");
    ...
    public static final Uri CONTENT_URI_SensingTime = Uri.parse("content://" + PROVIDER_NAME + "/SensingTime");
    ...
    private static final int SensingTime = 90;
    ...
    static {
        ...
        uriMatcher.addURI(PROVIDER_NAME, "SensingTime", SensingTime);
...
//                }
                ...
                case SensingTime:{
                    return dao.querySensingTime(projection,selection,selectionArgs);
                }
                case Phone_ACC:{
                    //if (selectionArgs != null && selectionArgs.length == 1 && selection == null) selection = "_id = ?";
...
//                }
                case SensingTime: {
                    Log.d(TAG, "CP insert: "+uri);
                    long id = dao.writeTime(DAO.NEW_SensingTime, values);
                    Log.d(TAG, "new id: " + id);
                    //showToast("DP: "+ id);
                    lastId = id;
                    return Uri.withAppendedPath(CONTENT_URI_SensingTime, id + "");
                }
                case E4_ACC: {
                    Log.d(TAG, "CP insert: "+uri);
...
//        }else
        if (match == SensingTime) {
            return insertInBulk(getDatabase(), DAO.SensingTime, values);
        }
